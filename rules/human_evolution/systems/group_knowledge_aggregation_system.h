...
//
// READS: Cognitive (memories), Agent (positions)
    ...
    void Update(SystemContext& ctx) override
    {
        ...
        for (size_t i = 0; i < qualifying.size(); ++i)
        {
            ...
            for (auto& rec : gk.records)
            {
                ...
                if (dist <= mergeRadius)
                {
                    gk.ReinforceRecord(rec.id, avgConfidence * reinforceFactor, currentTick);
                    reinforced = true;
                    break;
                }
            }
            ...
            if (!reinforced)
            {
                ...
                rec.contributors = static_cast<u32>(uniqueAgents.size());
            }
        }
    }
